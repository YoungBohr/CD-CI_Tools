...
def app_row(**meta):
    ...
    query = session.query(App).filter(App.name == name, App.type == app_type,
...
def repo_sync(**meta):
    ...
    def version_row():
        ...
        versions = session.query(Version).filter(Version.app_id == app_id).order_by(desc(Version.version))
        ...
        row = Version(app_id=app_id, version=version, commit_hash=head, commit_author=author,
